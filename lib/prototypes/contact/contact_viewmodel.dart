...
class ContactsViewModel extends FutureViewModel {
  ...
  String numberFormatter(String number) {
    ...
    final String formattedNumber = number.replaceAll('(', '').replaceAll(')','').replaceAll(RegExp(r"\s+"), "").replaceAll('-','').replaceAll(RegExp(countryCodePattern), '');
    log.wtf(stopwatch.elapsedMicroseconds);
